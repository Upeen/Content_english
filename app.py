...
def get_filters(key_prefix, include_hours=False):
    ...
    if filter_key not in st.session_state:
        st.session_state[filter_key] = {"date": datetime.date.today(), "source": "All", "min": None}
    ...
    with st.expander("🔍 Filters & Tools", expanded=True):
        ...
        with col3:
            if include_hours:
                lookback = st.number_input("Last - Min", min_value=1, max_value=1440, value=None, placeholder="Enter min", key=f"{key_prefix}_min")
            else:
                lookback = None
...
if page == PAGE_COVERAGE:
    ...
    for pair in source_pairs:
        ...
        if cov_start and cov_end:
            date1_in_range = False
            date2_in_range = False
            if ts1:
                date1 = ts1.date()
                if cov_start <= date1 <= cov_end:
                    date1_in_range = True
            if ts2:
                date2 = ts2.date()
                if cov_start <= date2 <= cov_end:
                    date2_in_range = True
            if not (date1_in_range and date2_in_range):
                continue
    ...
    if lookback_min is None:
        st.info("Enter Min value in 'Last - Min' filter and press Enter to see results.")
        st.stop()
